#复现省赛，使用ImageFolder按指定格式读取目录data_cla下的数据
import os
import torch
...
def build_loader(config):
    ...
    # 实例化训练数据集
    train_data_set = datasets.ImageFolder(root=os.path.join(config.DATA.DATA_PATH, "train"),
                                          transform=data_transform["train"])
    
    # 实例化验证数据集
    val_data_set = datasets.ImageFolder(root=os.path.join(config.DATA.DATA_PATH, "val"),
