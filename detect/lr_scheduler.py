...
def build_scheduler(config, optimizer, n_iter_per_epoch):
    ...
    elif config.TRAIN.LR_SCHEDULER.NAME == 'reduce_on_plateau':
        lr_scheduler = ReduceLROnPlateau(
            optimizer,
            mode='min',
            factor=config.TRAIN.LR_SCHEDULER.REDUCE_ON_PLATEAU.FACTOR,
            patience=config.TRAIN.LR_SCHEDULER.REDUCE_ON_PLATEAU.PATIENCE,
            min_lr=config.TRAIN.LR_SCHEDULER.REDUCE_ON_PLATEAU.MIN_LR,
            verbose=True
        )
    else:
        raise ValueError(f"Unsupported scheduler: {config.TRAIN.LR_SCHEDULER.NAME}")
